...
# ///
"""
Verus Analyzer Latest Release Downloader

Downloads the latest release of Verus Analyzer from GitHub releases.
...
def get_latest_release(include_prerelease=False):
    ...
    if include_prerelease:
        ...
        url = "https://api.github.com/repos/verus-lang/verus-analyzer/releases"
        ...
        url = "https://api.github.com/repos/verus-lang/verus-analyzer/releases/latest"
        response = requests.get(url)
        response.raise_for_status()
...
def setup_verus_analyzer_installation(binary_path, install_dir=None):
    """Set up Verus Analyzer installation in a clean directory."""
    binary_path = Path(binary_path)
    
    # Default install directory
    if install_dir is None:
        home_dir = Path.home()
        install_dir = home_dir / 'verus-analyzer'
    else:
        install_dir = Path(install_dir)
    
    print(f"Found verus-analyzer binary: {binary_path}")
    ...
    print(f"Installing Verus Analyzer to: {install_dir}")
    install_dir.mkdir(parents=True, exist_ok=True)
    
    # Copy binary to install directory with standard name
    installed_binary = install_dir / 'verus-analyzer'
    if platform.system() == 'Windows':
        installed_binary = install_dir / 'verus-analyzer.exe'
...
def update_path_in_shell_config(install_dir):
    """Add Verus Analyzer installation directory to PATH in shell configuration."""
    install_dir = Path(install_dir)
    config_file = get_shell_config_file()
    
    # Path export line to add
    path_line = f'export PATH="{install_dir}:$PATH"  # Added by Verus Analyzer installer'
    ...
    print(f"Adding Verus Analyzer to PATH in {config_file}")
...
def create_windows_batch_script(install_dir):
    """Create a batch script for Windows to add Verus Analyzer to PATH."""
    install_dir = Path(install_dir)
    batch_file = install_dir.parent / 'add_verus_analyzer_to_path.bat'
    
    batch_content = f'''@echo off
echo Adding Verus Analyzer to PATH...
setx PATH "%PATH%;{install_dir}"
echo Verus Analyzer has been added to your PATH.
...
'''
    ...
    print("Run this script as Administrator to add Verus Analyzer to your system PATH")
    return batch_file
...
def verify_installation(binary_path):
    """Verify that Verus Analyzer is working correctly."""
    print("Verifying Verus Analyzer installation...")
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"✓ Verus Analyzer is working! Version info:")
            print(result.stdout.strip())
            return True
        else:
            print(f"⚠ Verus Analyzer binary exists but returned error:")
            print(result.stderr.strip() if result.stderr else "Unknown error")
            return False
    except subprocess.TimeoutExpired:
        print("⚠ Verus Analyzer version check timed out")
        return False
...
def main():
    parser = argparse.ArgumentParser(description='Download and install the latest Verus Analyzer release')
    ...
    parser.add_argument('--install-dir', '-i',
                       help='Installation directory (default: ~/verus-analyzer)')
    ...
    try:
        ...
        if args.pre_release:
            print("Fetching latest Verus Analyzer pre-release...")
        else:
            print("Fetching latest stable Verus Analyzer release...")
        ...
        if args.vsix:
            # Look for VS Code extension for the platform
            ...
            for asset in assets:
                if asset['name'].endswith('.vsix'):
                    if not platform_pattern:
                        # If no platform specified, let user choose
                        vsix_assets = [a for a in assets if a['name'].endswith('.vsix')]
                        if len(vsix_assets) == 1:
                            vsix_asset = vsix_assets[0]
                        else:
                            print("Multiple VS Code extension assets available:")
                            for i, a in enumerate(vsix_assets):
                                print(f"  {i}: {a['name']}")
                            
                            choice = input("Enter asset number: ")
                            try:
                                vsix_asset = vsix_assets[int(choice)]
                            except (ValueError, IndexError):
                                print("Invalid choice")
                                return
                        break
                    elif platform_pattern in asset['name']:
                        vsix_asset = asset
                        break
            
            if not vsix_asset:
                print(f"No VS Code extension found for platform: {platform_pattern}")
            ...
            if platform_pattern:
                ...
                if not asset:
                    ...
                    for a in assets:
                        if a['name'].endswith('.gz') and not a['name'].endswith('.vsix'):
                            print(f"  - {a['name']}")
                    return
            else:
                binary_assets = [a for a in assets if a['name'].endswith('.gz') and not a['name'].endswith('.vsix')]
        ...
        if not args.no_extract:
            if filename.name.endswith('.gz'):
                try:
                    ...
                    # Set up installation
                    install_dir, installed_binary = setup_verus_analyzer_installation(
                        extracted_binary, args.install_dir
                    )
                    
                    print(f"✓ Verus Analyzer installed to: {install_dir}")
                    print(f"✓ Verus Analyzer binary: {installed_binary}")
                    ...
                    if not args.no_path:
                        ...
                        if platform.system() == 'Windows':
                            ...
                            print(f"   1. Run {config_file} as Administrator to add Verus Analyzer to PATH")
                            print(f"   2. Restart your command prompt/PowerShell")
                            print(f"   3. Type 'verus-analyzer --version' to verify")
                        else:
                            print(f"\n📝 Next steps:")
                            print(f"   1. Restart your terminal or run: source {config_file}")
                            print(f"   2. Type 'verus-analyzer --version' to verify")
