...
def setup_rust_analyzer_installation(binary_path, install_dir=None):
    ...
    if install_dir is None:
        home_dir = Path.home()
        install_dir = home_dir / 'rust-analyzer'
    else:
...
def update_path_in_shell_config(install_dir):
    """Add Rust Analyzer installation directory to PATH in shell configuration."""
    install_dir = Path(install_dir)
    config_file = get_shell_config_file()
    
    # Path export line to add
    path_line = f'export PATH="{install_dir}:$PATH"  # Added by Rust Analyzer installer'
    ...
    print(f"Adding Rust Analyzer to PATH in {config_file}")
...
def main():
    ...
    parser.add_argument('--install-dir', '-i',
                       help='Installation directory (default: ~/rust-analyzer)')
